...
const uk_url: ShopUrl = {
    url_shop_info: 'https://www.amazon.co.uk/Computer-Desk-inches-Writing-Frame%EF%BC%8CBrown/dp/{ASIN}/ref=cm_cr_arp_d_product_top?ie=UTF8&deliveryCountryCode=GBP',
...
async function reqShopInfoByUrl(asin: string, url: string, domain: string): Promise<ShopInfo> {
    ...
    try {
        ...
        try {
            ...
            if (error) {
                console.log(new Date().toLocaleString() + ' error 获取review出错 ' + error.message + ' [asin = ' + asin + ']');
            }
        }
        return shopInfo;
    } catch (err) {
